...
class AdapterRealWorld {
    ...
    ///
    /// Firstly, you have to create the new protocol 'AuthService' and insert the authorization method of Facebook SDK.
    ///
    /// Secondly, write an extension for Twitter SDK and implement methods of AuthService protocol, just a simple redirect.
    ///
    /// Thirdly, write an extension for Facebook SDK. You should not write any code at this point as methods already implemented by Facebook SDK.
    ///
...
extension FacebookAuthSDK: AuthService {
    /// This extension just tells a compiler that both SDKs have the same interface.
}
...
extension TwitterAuthSDK: AuthService {
    /// This is an adapter
    ///
    /// Yeah, we are able to not create another class and just extend an existing one
